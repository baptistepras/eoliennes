...
void afficheFils() {
  ...
  for (int i = 0; i < nbPylones-1; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      //maintenant les fils entre les pylones et eoliennes   
      ...
      if (i < 6){
         ...
         float eolDroitePosz = eoliennes[i+6].position.z + pointAncrage.z;
          ...
          pylones[i].position.x+ filsBas[2].x, pylones[i].position.y+ filsBas[2].y, pylones[i].position.z+ filsBas[2].z);
...
public void setup() {
  pointAncrage = new Vector(0, -1.1*4/5, 4);
